...
docker run --rm \
  ...
  -v "$(pwd)/order:/go/src/order" \
  -w /go/src/order \
  golang:1.21 \
  sh -c "go mod download && go test -v ./... | tee /test-results/order-test-output.txt" || echo "Order tests failed but continuing..."
...
docker run --rm \
  ...
  -v "$(pwd)/test-results:/test-results" \
  -v "$(pwd)/payment:/go/src/payment" \
  -w /go/src/payment \
  golang:1.21 \
  sh -c "go mod download && go test -v ./... | tee /test-results/payment-test-output.txt" || echo "Payment tests failed but continuing..."
